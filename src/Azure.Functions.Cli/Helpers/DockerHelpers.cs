...
namespace Azure.Functions.Cli.Helpers
{
    public static class DockerHelpers
    {
        ...
        public static async Task<string> DockerRun(string image, string entryPoint = null)
        {
            var command = $"run --rm -d -it {(entryPoint != null ? $"--entrypoint {entryPoint}" : string.Empty)} {image}";
            (var output, _, _) = await RunDockerCommand(command, showProgress: false);
        ...
        internal static async Task<TriggersPayload> GetTriggersFromDockerImage(string imageName)
        {
            ...
            try
            {
                containerId = await DockerRun(imageName, "/bin/sh");
